...
"""
分布式：一个主机的redis作为主体，在所有的子机上部署scrapy代码，并配置好redis连接，之后直接在主机上执行爬取的网站，所有子机从redis队列中取。
注意：怎样防止中断？
...
分布式爬虫的运行原理：
	...
	3：爬虫引擎将调度器中的任务直接提取提交给处于空闲状态的下载器
	4：下载器将数据下载之后提交给对应爬虫引擎，爬虫引擎此时将结果传递给spider
